...
class InfiniteQueryOptions<TData, TError, TPageParam> extends QueryOptions<TData, TError> {
...
class InfiniteQueryObserver<TData, TError, TPageParam> extends ChangeNotifier with QueryListener {
  ...
  InfiniteQueryObserver(
    ...
    query = client.queryCache.build<InfiniteQueryData<TData, TPageParam>, TError>(
      queryKey: queryKey,
      client: client,
    );
    _setOptions(options);
    ...
    if (isRefetching && !isInvalidated) {
      switch (options.refetchOnMount) {
        ...
        case RefetchOnMount.stale:
          DateTime? staleAt = query.state.dataUpdatedAt?.add(options.staleDuration);
  ...
  void _setOptions(UseInfiniteQueryOptions<TData, TError, TPageParam> options) {
    this.options = InfiniteQueryOptions<TData, TError, TPageParam>(
      ...
      refetchOnMount: options.refetchOnMount ?? client.defaultQueryOptions.refetchOnMount,
      staleDuration: options.staleDuration ?? client.defaultQueryOptions.staleDuration,
      cacheDuration: options.cacheDuration ?? client.defaultQueryOptions.cacheDuration,
  ...
  void updateOptions(
    ...
    final refetchIntervalChanged = this.options.refetchInterval != options.refetchInterval;
    ...
    pageParams.forEachIndexed((i, param) async {
      ...
      await resolver.resolve<TData>(
        ...
        onResolve: (refetchedData) {
          ...
          final action = isLastPage ? DispatchAction.success : DispatchAction.refetchSequence;
    ...
    await resolver.resolve<TData>(
      ...
      onResolve: (data) {
        ...
        final newData = query.state.data?.copyWith(
              pages: meta.direction == FetchDirection.forward ? [...pages, data] : [data, ...pages],
              pageParams: meta.direction == FetchDirection.forward ? [...pageParams, pageParam] : [pageParam, ...pageParams],
            ) ??
