...
class QueryClient {
  ...
  void setQueryData<TData>(
    ...
    final query = queryCache.build<TData, dynamic>(queryKey: queryKey, client: this);
  ...
  void invalidateQueries(QueryKey key, {bool exact = false}) {
    queryCache.queries.forEach((queryKey, query) {
      ...
      if (exact) {
        ...
        final isPartialMatch = queryKey.length >= key.length && queryKey.sublist(0, key.length) == key.lock;
  ...
  int isFetching() {
    return queryCache.queries.entries.where((queryMap) => queryMap.value.state.isFetching).length;
  }
}
